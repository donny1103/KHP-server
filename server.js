...
PENDING_USERS = {
  ...
  queue: {
    ...
    // '0243cf5e-98d3-4e17-1234-151esd7e2342': { age: 10, name: 'Jacob', gender: 'male', 'severity': 60, time: new Date(), sadValue: 3, scaredValue: 3, favoriteColor: 'pink' , careAbout:['brother','dad','mom']},
    // '0243cf5e-98d3-4e17-1234-151e8s7e2342': { age: 6, name: 'Kani', gender: 'female', 'severity': 33, time: new Date(), sadValue: 2, scaredValue: 7, favoriteColor: 'red' , careAbout:['dad','mom']},
    // '0243cf5e-98d3-4e17-1234-dd1e8b7e2342': { age: 7, name: 'Niveditha', gender: 'female', 'severity': 28, time: new Date(), sadValue: 6, scaredValue: 6, favoriteColor: 'purple' , careAbout:['sister','brother','dad','mom']},
  },
  counsellorId:''
};
  ...
  wss.clients.forEach(client => {
    if (client.readyState === WebSocket.OPEN) {
      ...
      if (action === 'excluded' && id !== client.id) {
        client.send(data);
        console.log('Broadcase exclude:', id)
      }
    }
  })
};
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', data => {
    ...
    switch (message.type) {
      ...
      case 'startChat':
        console.log(`${message.counsellor.name} start Chat  with ${message.name ? message.name : message.userId}`);
